...
class Game:
    ...
    def auto_control(self):
        ...
        # Define all four possible moves with corresponding new coordinates
        directions = {
        ...
        # Avoid reversing direction
            ...
            # Check if the move would lead to collision
            if self._would_collide(new_x, new_y):
                continue

            # Calculate Euclidean distance to apple (goal)
            distance = math.hypot(apple_x - new_x, apple_y - new_y)
            safe_moves.append((direction, distance))

        # Choose the move that minimizes distance to apple (goal-based decision)
        if safe_moves:
